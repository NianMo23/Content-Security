...
from qwen3_original_model import Qwen3OriginalForClassification
...
def evaluate(model, dataloader, device, rank, world_size, tokenizer, use_fp16=True):
    model.eval()
    all_preds = []
    all_labels = []
    total_loss = 0
...
def test_worker(rank, world_size, args):
    ...
    base_model = Qwen3OriginalForClassification(args.checkpoint)
    ...
    test_results = evaluate(model, test_loader, device, rank, world_size, tokenizer, use_fp16=args.fp16)
    ...
    if is_main_process and test_results and 'accuracy' in test_results:
        ...
        results_file = os.path.join(output_dir, f'original_multi_gpu_test_results_{timestamp}.json')
        ...
        with open(results_file, 'w', encoding='utf-8') as f:
            json.dump({
                'test_config': {
                    ...
                    'fp16': args.fp16,
                    'timestamp': timestamp,
                    'model_type': 'original_qwen3_with_lora'
                },
                'results': {
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str, required=False, default="/home/users/sx_zhuzz/folder/LLaMA-Factory/mymodels/Qwen3-1.7B",
                        help="原始模型的路径")
    parser.add_argument('--lora_model', type=str, required=False, default="../original-model-output/best_model",
                        help="保存的LoRA模型路径")
    parser.add_argument('--test_data', type=str, required=False, default="../data/r2323-b-50000.xlsx",
                        help="测试数据集路径")
