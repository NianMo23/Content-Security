...
class Qwen3ForSequenceClassification(nn.Module):
    ...
    def __init__(self, checkpoint, num_labels=6):
        ...
        # 加载预训练的Qwen3模型
        print("正在加载Qwen3模型...")
        # 不使用device_map="auto"，让DDP自己管理设备分配
    ...
    def predict_with_confidence_threshold(self, logits, normal_threshold=0.35):
        """
        自定义预测方法：如果类别0（正常）的置信度高于阈值，则归类为正常
        
        Args:
            logits: 模型输出的logits [batch_size, num_labels]
            normal_threshold: 正常类别的置信度阈值，默认0.35
            
        Returns:
            predictions: 预测的类别标签 [batch_size]
        """
        # 计算softmax概率
        ...
        # 应用置信度阈值规则：如果正常类别概率 > 阈值，则预测为正常
        final_preds = torch.where(
            normal_probs > normal_threshold,
            torch.zeros_like(argmax_preds),  # 预测为正常（类别0）
            argmax_preds  # 否则使用argmax结果
        )
        
        return final_preds, probabilities